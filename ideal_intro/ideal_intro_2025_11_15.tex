...
In markets with asymmetric information, consumers may use initial price offers as leverage to obtain better prices. The welfare and distributional effects of this consumer search, however, are ambiguous. If searching signals lower search costs or higher price elasticity, firms can best respond by offering higher initial prices. This description fits the markets for retirement annuities, personal loans, auto insurance, and housing, among others. Despite potentially significant effects on welfare and competition, there is limited evidence on the size and magnitude of this “search and leverage” effect. We show that identifying the joint distribution of search costs and price elasticity is critical for understanding the effects of search and leverage. Using data from the Chilean annuities market, we estimate this distribution and show that the Chilean regulator’s decision to forbid the search for revised offers increased welfare by 11\%, with significant distributional effects.
...
Consider a market where consumers receive offers from one or more sellers, and then use those offers to negotiate better prices. 

\newpage

In our setting all consumers are first time buyers. No switching, no complementary products/cross-selling. No need to restrict to ``first-time buyers'' like \textcite{cosconati_competing_2025} (where it is hard to even define what a first time buyer is).

\noindent So, there are none of the following three confounders:
\begin{itemize}
    \item Previous history
    \item No switching (so, no selling at low price today to behavioral types, just to raise prices in the future)
    \item No cross-selling
\end{itemize}
