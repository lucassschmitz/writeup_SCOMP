...
\begin{itemize}
    ...
    \begin{itemize}
        \item Permits discrimination based on search costs 
...
\begin{frame}{Setting: annuities}\label{slide:setting}
    ...
    %\begin{align*}
    %\pi_{ji}(F) = S_i-  \sum_{t=1}^T Pr_j (\text{alive}|x_i)\frac{F}{(1+r_j)^t}
    %\end{align*}

    \begin{align*}
    \pi_{ji}(F) = S_i -  \sum_{t=1}^T \frac{\Pr_j(\text{alive} \mid x_i)}{(1+r_j)^t} F 
    \end{align*}
...
\begin{itemize}
    ...
    \item Each firm $j$ observes a noisy signal $\hat \theta_j \sim \mathcal{N}(\theta, \sigma_j^2)$ with pdf $\phi(\hat \theta_j; \theta, \sigma_j)$ before posting its initial offer $F_j^{1T}$.
    \item Buyers draw search cost $s_i$ from a joint distribution $F(s,\theta)$    \item Timeline:
    \begin{enumerate}
...
\small
\begin{align*}
E(\theta \mid \hat \theta_j, D=j, S=0) &= \frac{\int \theta \, 
%\textcolor{blue}{\widehat D_{j0}(\theta, \hat \theta_j)}
\textcolor{blue}{\Pr(D = j, S= 0 \mid \theta, \hat \theta_j)}
...
E(\theta \mid \hat \theta, D=j, S=1) &= \frac{\int \theta \, 
%\widehat D_{j1}(\theta, \hat \theta) 
\textcolor{blue}{\Pr(D = j, S= 1 \mid \theta, \hat \theta)}
...
\small
\begin{align*}
    \pi_j &= \int_{\theta} \int_{\hat \theta} (W - F_j^{1T}(\hat \theta_j)  k_j \theta) \, \Pr(D=j \mid \theta, \hat \theta, S=0) (1-\pi_1(\theta, \hat \theta)) \, \phi(\hat \theta; \theta) f_0(\theta) d\hat \theta d\theta \\
    &\quad + \int_{\theta} \int_{\hat \theta} (W - F_j^{2T}(\hat \theta) k_j \theta) \, \Pr(D=j \mid \theta, \hat \theta, S=1) \pi_1(\theta, \hat \theta) \, \phi(\hat \theta; \theta) f_0(\theta) d\hat \theta d\theta.
\end{align*}
\normalsize
\begin{itemize}
