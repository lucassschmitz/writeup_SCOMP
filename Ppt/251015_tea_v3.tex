...
\begin{frame}{Second Stage: optimal pricing}

\begin{itemize}
    \item When selected for revised offer, firm observes competitors' initial prices
    \item After observing competitors, firm best-responds to known prices rather than expected prices
\end{itemize}

\vspace{0.3cm}

\textbf{Optimal updated offer:}
$$p^{T2}_j(c_j, p^{T1}) = \min(p^{T1}_j, p^*(c_j, p^{T1}_{-j}))$$

where
$$p^*(c_j, p^{T1}_{-j}) = \arg\max_{p_j} (p_j - c_j) D_j(p_j, p^{T1}_{-j})$$
\end{frame}

%%%%%

\begin{frame}{Second stage: profits}
\begin{itemize}
    \item When consumer searches, firm $j$ faces two scenarios:
\end{itemize}
\begin{enumerate}
    \item Selected for revised offer ($\frac{1}{J}$ probability):
    $$\pi^{(j)}_j(p^{T1}, c_j) = (p^{T2}_j(c_j, p^{T1}) - c_j) D_j(p^{T2}_j(c_j, p^{T1}), p^{T1}_{-j})$$
    \item Competitor $j'$ selected ($\frac{1}{J}$ probability):
...
\begin{itemize}
    \item Firms anticipate the second stage when setting initial prices
    \item Expected profits in the first stage:
...
\begin{frame}{Simulations: equilibrium strategies}
\begin{figure}[H]
\centering{}%
\begin{tabular}{cc}
\includegraphics[scale=0.5]{../figures/simulations/model5/pricesCI_IC.png}
\end{tabular}
\end{figure}
\begin{itemize}
    \item Parameters: $J= 4, \alpha = 1, \delta = 2, c_j \sim N(2, 0.5^2)$  
\end{itemize}
\end{frame}
...
\begin{itemize}
    ...
    \begin{align*}
        ...
        = \left(S_i- \mathbb{E}^j_{T} \left[\sum_{t=1}^T\frac{F_{ij}}{(1+r_j)^t}|x_i \right]\right)\mathds{1}(\text{i chooses j}) 
    \end{align*}
\end{itemize}
