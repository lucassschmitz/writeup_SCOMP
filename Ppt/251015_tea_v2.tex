...
\begin{center}
\input{../figures/tikz/SCOMP_diagram_v2.tex}
\end{center}
\begin{itemize}
...
Explain that: 
\begin{itemize}
    \item Mention that external offers are called this way because they are external to the system, moreover they are less regulated
    \item An exception to less regulation is that they can not be lower than initial offers. 
    \item only initial bidders can make an external offer 
    %\item We do not observe requests only the external offers. 
...
\note{
    \begin{itemize}
        \item I observe only the external offers made not the requested ones. 
