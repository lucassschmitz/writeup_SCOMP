...
\begin{align}
    ...
    \phi(\hat \theta_{-j}; \theta) d\hat \theta_{-j}
\end{align}
and equivalently for searchers, define: 
...
\begin{align}
    ...
    &= \int_\theta \theta f(\theta\mid \hat \theta_j, D=j, S=0) d\theta \notag \\
    &= \int_\theta \theta 
    \frac{  Pr(D = j, S = 0  \mid \theta, \hat \theta_j) \cdot  f(\theta \mid \hat \theta_j, S=0)     }{Pr(D= j \mid \hat \theta_j, S = 0)}
    d\theta \notag \\ 
    &= \int_\theta \theta 
    \frac{ D_{j0}(\theta, \hat \theta_j) \cdot f(\theta \mid \hat \theta_j, S=0)     }{Pr(D= j \mid \hat \theta_j, S = 0)}
    d\theta 
...
&= \frac{\displaystyle
       ...
       {\displaystyle
        \int
             \Pr\!\left(D=j, S=0 \mid \theta', \hat\theta_j\right)\,
             f\!\left(\theta' \mid \hat\theta_j\right)\, d\theta'}
&&\text{(factor common denominator outside integral).}
