...
%The utility of accepting a price equal to the reservation price \(R\) must equal the expected gain from one additional search.  Hence
%\begin{align*}
%v - R &=F(R)\Bigl[\int_{0}^{R}(v - p)\,dF(p) - s\Bigr] +F(R)\bigl[1 - F(R)\bigr]\Bigl[\int_{0}^{R}(v - p)\,dF(p) - s\Bigr]\\ &\quad+\bigl[1 - F(R)\bigr]^{2}F(R)\Bigl[\int_{0}^{R}(v - p)\,dF(p) - s\Bigr] +\cdots.
%\end{align*}

%Define
%\[
%M(R)=\int_{0}^{R}(v - p)\,dF(p) - s.
%\]

%The optimality condition becomes
%\begin{align*}
%v - R &=F(R)\,M(R)\bigl[1 + (1-F(R)) + (1-F(R))^{2} + \cdots\bigr]\\
%&=F(R)\,M(R)\,\frac{1}{1 - (1-F(R))} =M(R).
%\end{align*}
