...
\begin{itemize}
  ...
  \item Evidence from Italian auto insurance using a panel linking consumers across insurers.
  \item Counterfactuals: centralized risk bureau, full information, and data privacy regulation. 
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{enumerate}
  ...
  \item Antitrust/Consumer protection and Big Data  
  \begin{itemize}
...
\begin{frame}{Institutional Background: Italian Auto Insurance}
\justifying
\begin{itemize}
  ...
  \item Key contract features widely standardized; little use of deductibles and full coverage
\end{itemize}
\end{frame}
...
\justifying
\begin{itemize}
  \item Nationally representative matched insurer--insuree panel
  \begin{itemize}
      \item Observable risk factors
      \item Premiums 
      \item Frequency and severity of claims 
  \end{itemize}
  \item Tracks policyholders across insurers and time
...
% 10 - Price variation and Sorting
\begin{frame}{Price Variation \& Sorting}
\begin{figure}[H]
\centering{}%
\begin{tabular}{cc}
...
\vspace{0cm}
\begin{itemize}
  \item Substantial cross-firm variation in: 
  \begin{itemize}
      \item Prices, even at similar risk/market share. (e.g. F4 and F5). 
      \item Average claim payouts (e.g. F3 and F8)
  \end{itemize}
  
  \item Variation in claim payouts suggests sorting. 
\end{itemize}
\end{frame}

% 11 - Measuring Information Precision
\begin{frame}{Measuring Information Precision}\label{slide:measuring}
\begin{enumerate}
\item  Construct individual risk measures
\begin{itemize}
...
\vspace{.1cm}
\item $X_{it}$ includes age, vehicle features, geographic location and contractual clauses. 
\end{itemize}
\item Firm-specific premium-risk regressions
\begin{itemize}
\item For firm $j$: 
$$\text{Premium}_{ij,t=1} = \alpha_j + \beta_j \cdot \text{Risk}_{i,t=1} + \varepsilon_{ij,t=1}$$
\item Higher $\beta_j$ suggests firm's prices are more responsive to actual risk
\end{itemize}
\end{enumerate}
 \hyperlink{slide:simple_example}{\beamergotobutton{Example}}
\end{frame}
...
\begin{itemize}
  ...
  \item $D =j$: consumer chooses firm $j$. 
\end{itemize}
\end{frame}
...
\end{align}
%\item Without selection: 
%\begin{align}
%  E(\theta | \hat{\theta}_j, D=j) = E(\theta | \hat{\theta}_j)=
%    \frac{\hat{\theta}_j\sigma_0^2+ \mu_0\sigma_j^2}{\sigma_j^2+ \sigma_0^2}
%\end{align}
\end{itemize}
...
\justifying
\begin{itemize}
  \item Consumers choose one insurer (no outside option)
  \item Preference parameters allowed to vary with  risk type. Consumer $i$'s utility from firm $j$:
\end{itemize}
\begin{align}
...
\begin{frame}{Firm profits}
    \begin{itemize}
        ...
        \[\pi_j(\alpha, \beta) = 
        \int_{\hat{\theta}} \int_{\theta} \underbrace{(p_j(\hat{\theta}_j) + c_j - k_j\theta)}_{\text{net profit}} \underbrace{\Pr(D=j|\hat{\theta}, \mathbf{\theta})}_{\text{choice prob.}} \underbrace{\left(\prod_{j'=1}^{J} \phi(\hat{\theta}_{j'}; \theta, \sigma_{j'}) \right)}_{\text{signal dist.}} \underbrace{f_0(\theta)}_{\text{type dist.}} d\theta d\hat{\theta}.
        \]
        ...
        %\item Equilibrium exists, but is not necessarily unique. 
    \end{itemize}
\end{frame}
...
\begin{frame}{Estimation Overview}\label{estimation:overview}
  ...
  \end{itemize}
    ...
    \begin{itemize}
      \item Average price and covariance of price and risk identify $(\alpha_j, \beta_j)$  \hyperlink{appendix:slide3}{\beamergotobutton{Details}}
...
\vspace{0.3cm}
\textbf{Solution:} use selection rule (demand model) to recover distribution of offered prices
%\begin{itemize}
...
{\small
Relationship:
$$ \underbrace{g(p|\theta, D=j)}_{\text{Accepted prices}} \propto \underbrace{g_j(p|\theta)}_{\text{Offered pricess}} \Pr(D=j|p_j=p, \theta) $$
...
\begin{frame}{Iterative Algorithm for Demand Estimation}\label{slide:iterative}
\small 
...
\begin{enumerate}
    \item Initialize: $\xi_j^1 = 0$ and $\Pr^1(D=j|p_j=p, \theta) = \exp(-\gamma p)$ for all $j$
    ...
    {\small
    $$ \hat{s}_j = \int_\theta \int_p \frac{\exp(\gamma p_j + \xi_j^{r+1})}{\sum_{j'} \exp(\gamma p_{j'} + \xi_{j'}^{r+1})} \left(\prod_{j'} g_{j'}^r(p_{j'}|\theta)\right) f_0(\theta) dp d\theta $$
    }
    
    \item Update selection probabilities $\Pr^{r+1}(D=j|p_j=p, \theta)$ using demand model \hyperlink{appendix:slide2}{\beamergotobutton{Details}}
...
\textbf{Outer Loop:} Estimate $\gamma$ by matching risk sorting patterns via maximum likelihood

\end{frame}

\begin{frame}{Outer Loop}
\begin{itemize}
    \item Outer loop is over $\gamma$: 
\end{itemize}

\begin{align}
    LL_d(\gamma) = \sum_{i=1}^{N} \sum_{j=1}^{J} \mathbf{1}\{D_i = j\} \log \left( \Pr(D_i = j | \tilde{\theta}_i; \gamma) \right),
\end{align}
where demand comes from the inner loop: 
\begin{align}
...
\centering{}%
\begin{tabular}{cc}
\includegraphics[scale=0.53]{Figures/Tab5.png}
\end{tabular}
\end{figure}
\end{frame}
...
\begin{itemize}
  \item This paper develops empirical framework for studying competition under information asymmetries 
...
\begin{itemize}
  \item What is the impact of a credit bureau if there is learning and switching frictions?  
  \begin{itemize}
    \item Cost of risk miss-classification is low -> can update prices. 
    \item Efficient firms might invest in learning, not in reducing $\sigma_j$
    \item $c_j$ should be risk specific to capture inertia? 
  \end{itemize}
  \item What does a credit bureau do?  
  \begin{itemize}
...
\begin{itemize}
  ...
  \]
    ...
    \hyperlink{slide:iterative}{\beamergotobutton{Go back: iterative algorithm}}
\end{itemize}
\end{frame}
...
%    \item Assume
%    \begin{itemize}
%        \item Claim count $\in \{0,1\}$ depending on a contractual clause $X_{it}\in \{0,1\}$. Buyers who (do not) include the clause, (do not) have accidents.
%        \item Claim Size is always 1. 
%    \end{itemize}
%    \item $i$ does not buy the clause the first period, but does the $T-1$ periods after that. 
%
%    \item $i$'s risk measure 
%\end{itemize}
% \hyperlink{slide:simple_example}{\beamergotobutton{Example}}
%\end{frame}
%\begin{frame}{Simple example}\label{slide:simple_example}
%\begin{itemize}
%    \item Buyer risk is  $\theta_i = x_1 + x_2$, the number of accidents (assume size is always 1) is $\theta_i + u_i$, with $u_i$ iid. 
%    \item There are two firms (A,B)
%    \item The econometrician and A  observe $x_1$. B observes $x_1$ and $x_2$. 
    
%    \item Both firms have the same strategy, they price at the observed risk, $p_A = x_1, p_B = x_1+x_2$.
    
    
    %Hence $\beta_1 =1+\frac{\text{cov}(x_1,x_2)}{\text{var}(x_1)}, \beta_2 =1  $
    %\item The econometrician concludes $\hat{\beta}_A = 1$, $\hat{\beta}_B =1+\frac{\text{cov}(x_1,x_2)}{\text{var}(x_1)} $
    %\item If $\text{covar}(x_1,x_1) < 0$ one concludes that $B$ is more precise. 
%\end{itemize}
% \hyperlink{slide:measuring}{\beamergotobutton{Go back}}
%\end{frame}


\begin{frame}{Simple example}\label{slide:simple_example}
\begin{itemize}
    \item There is no uncertainty, and real risk is $\theta_i = x_1 + x_2$. 
    \item There are two firms (A,B)
    \item The econometrician and A  observe $x_1$. B observes $x_1$ and $x_2$. 
    \item Both firms have the same strategy, they price at the observed risk, $p_A = x_1, p_B = x_1+x_2$. Hence $\beta_1 =1+\frac{\text{cov}(x_1,x_2)}{\text{var}(x_1)}, \beta_2 =1  $
    \item The econometrician concludes $\hat{\beta}_A = 1$, $\hat{\beta}_B =1+\frac{\text{cov}(x_1,x_2)}{\text{var}(x_1)} $
    \item If $\text{covar}(x_1,x_1) < 0$ one concludes that $B$ is more precise. 
\end{itemize}
 \hyperlink{slide:measuring}{\beamergotobutton{Go back}}

If the dist. of consumers over years is uniform, they have an average of 4 years for each consumer and a claim rate of .08. Probably noisy fixed effects in the risk regressions.
\end{frame}


\begin{frame}{Capturing markups, not precision example}\label{slide:simple_example}
\begin{itemize}
    \item The model estimates $\hat{\theta_i}=\theta+v_i$.
    
    \item The model fit could be low (few covariates, short panel and low likelihood of occurrence)

    \item If firms observe the true risk, but they have different pricing strategies $p_{ij} = \alpha_j+\beta_{j}\theta_i$ then: 

    $$ \hat{\beta}_j=\beta_j\frac{\text{var}(\theta_i)}{\text{var}(\theta_i)+\text{var}(v_i)}$$   
