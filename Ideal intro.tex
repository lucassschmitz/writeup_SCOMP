...
\begin{itemize}
    ...
    In markets where prices are set for each individual it is common for consumers to get initial offers which they can leverage to get revised offers. For example, when requesting a loan, banks make initial offers to customers and give them a Loan Estimate (LO) which is a document with the terms of the offer (see Figure \ref{fig:LE_example}). Then customers can use the LO from one bank to improve the terms of the offer from another bank.
