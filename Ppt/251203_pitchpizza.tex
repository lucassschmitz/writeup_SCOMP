...
\begin{center}
\input{../figures/tikz/SCOMP_diagram_v3.tex}
\end{center}
\begin{itemize}
...
\begin{enumerate}
    ...
    \item Sorting into firms $\rightarrow$ algorithm precision?  \hyperlink{slide:fig3}{\beamerbutton{Sorting into firms}}
...
\begin{column}{0.48\textwidth}
     \begin{itemize}
        \item     If firms do not know competitors' prices one expects them to increase their offers more when the competitors' prices are higher. 
    $$ F_{ij}^R - F_{ij}^I = \beta_0 + \beta_1 \text{Avg. gap}_{ij} + \beta_2 \text{Max. gap}_{ij} + \beta_j + \epsilon_{ij}$$
    where  \\
    $ \text{Avg. gap }_{ij} = \frac{1}{F_{ij}^I}\left(\frac{1}{J-1}\sum_{k \neq j} F_{ik}^I - F_{ij}^I\right) $\\ 
    $ \text{Max. gap } = \frac{1}{F_{ij}^I}\left(\max_{k \neq j} F_{ik}^I - F_{ij}^I\right)$
    \item How to deal with selection (observe revisions not requests)? 
\end{itemize}
\end{column}
\hfill
\begin{column}{0.48\textwidth}
...
\begin{itemize}
    \item $J$ retirement insurers compete in a centralized marketplace.
    \item Buyer type $\theta$ (mortality risk) is private; prior density $f_0(\theta)$.
    \item Each firm $j$ observes a noisy signal $\hat \theta_j \sim \mathcal{N}(\theta, \sigma_j^2)$ before posting its initial offer $F_j^{1T}$.
    \item Buyers draw search cost $s_i$ from a joint distribution $F(s,\theta)$ that can correlate search propensity with risk.
    \item Timeline:
    \begin{enumerate}
        ...
        \item Upon search, all firms observe the full vector $\hat \theta$ through posted prices and can update offers.
    \end{enumerate}
\end{itemize}
\end{frame}
...
\end{itemize}
\begin{align*}
    F_j^{1T}(\hat \theta_j) &= \alpha_j + \beta_j \, E[\theta \mid \hat \theta_j, D=j, S=0], \\
    F_j^{2T}(\hat \theta) &= \alpha_j^{2T} + \beta_j^{2T} \, E[\theta \mid (\hat \theta_j, \hat \theta_{-j}), D=j, S=1].
\end{align*}
\begin{itemize}
...
\begin{frame}{Demand and Search Decision}
\begin{itemize}
    \item Indirect utility for type $\theta$ from firm $j$ in stage $s \in \{0,1\}$:
    \begin{align*}
        u_{ij}^{(s)} = \gamma(\theta) F_j^{sT} + \xi_j + \beta r_j + \epsilon_{ij}.
    \end{align*}
    \item Logit demand conditional on search choice:
    \begin{align*}
        \Pr(D=j \mid \hat \theta, \theta, S=s) = \frac{\exp(\gamma(\theta) F_j^{sT}(\hat \theta) + \xi_j + \beta r_j)}{1 + \sum_k \exp(\gamma(\theta) F_k^{sT}(\hat \theta) + \xi_k + \beta r_k)}.
    \end{align*}
    \item Search occurs when the expected gain from revised offers exceeds the idiosyncratic cost:
    \begin{align*}
        G(\theta, \hat \theta) &= U_{i1}(\theta, \hat \theta) - U_{i0}(\theta, \hat \theta) > s_i, \\[-0.3em]
...
\small
\begin{align*}
E(\theta \mid \hat \theta_j, D=j, S=0) &= \frac{\int \theta \, \widehat D_{j0}(\theta, \hat \theta_j) \, \varphi(\hat \theta_j; \theta, \sigma_j) f_0(\theta) d\theta}{\int \widehat D_{j0}(\theta', \hat \theta_j) \, \varphi(\hat \theta_j; \theta', \sigma_j) f_0(\theta') d\theta'}, \\[0.4em]
E(\theta \mid \hat \theta, D=j, S=1) &= \frac{\int \theta \, \widehat D_{j1}(\theta, \hat \theta) \, \phi(\hat \theta; \theta) f_0(\theta) d\theta}{\int \widehat D_{j1}(\theta', \hat \theta) \, \phi(\hat \theta; \theta') f_0(\theta') d\theta'}.
\end{align*}
\normalsize
\begin{itemize}
    \item Terms $\widehat D_{js}$ weight beliefs by the probability the consumer chooses firm $j$ in stage $s$.
\end{itemize}
\end{frame}
...
\small
\begin{align*}
    \pi_j &= \int_{\theta} \int_{\hat \theta} (F_j^{1T}(\hat \theta_j) - k_j \theta) \, \Pr(D=j \mid \theta, \hat \theta, S=0) (1-\pi_1(\theta, \hat \theta)) \, \phi(\hat \theta; \theta) f_0(\theta) d\hat \theta d\theta \\
    &\quad + \int_{\theta} \int_{\hat \theta} (F_j^{2T}(\hat \theta) - k_j \theta) \, \Pr(D=j \mid \theta, \hat \theta, S=1) \pi_1(\theta, \hat \theta) \, \phi(\hat \theta; \theta) f_0(\theta) d\hat \theta d\theta.
\end{align*}
\normalsize
\begin{itemize}
    \item Firms choose $(\alpha_j, \beta_j, \alpha_j^{2T}, \beta_j^{2T})$ anticipating both selection (risk composition) and search (information revelation).
    \item Equilibrium trades off the profit erosion from better information in the aftermarket with the option value of serving non-searchers at higher markups.
\end{itemize}
\end{frame}

%%%%%

\begin{frame}{Mechanisms and Predictions}
\begin{itemize}
    \item \textbf{Information aggregation}: searchers force firms to pool their signals, eliminating winner's-curse concerns and intensifying competition for that pool.
    \item \textbf{Screening channel}: correlation between $s_i$ and $\theta$ means the act of searching signals risk type, changing the composition of non-searchers.
    \item If low-risk consumers search, the aftermarket improves selection and disciplines initial offers for everyone.
    \item If high-risk consumers search, revised offers may worsen selection and can even collapse the search market.
    \item Policy experiments (e.g., banning revised offers) affect both price dispersion and the endogenous sorting of types across search decisions.
\end{itemize}
\end{frame}
...
\begin{itemize}
    \item Firms cost: 
    \begin{align*}
        c_{ij} =  \mathbb{E}^j_{T} \left[\sum_{t=1}^T\frac{1}{(1+r_j)^t} \, \middle| \, x_i \right]
    \end{align*}  
    \item Firm prices: $p_j = S_i/F_{ij}$
    \item Demand: $ D_{ij}(F_{ij}) = F_{ij}\mathds{1}(\text{i chooses j})  $
    \item Firm profits: 
    \begin{align*}
        \pi_{ij}(F) = (p_j - c_{ij}) D_{ij}(F_{ij})  %= \left(\frac{S}{F_{ij}}-   \mathbb{E}^j_{T} \left[\sum_{t=1}^T\frac{1}{(1+r_j)^t}|x_i \right]\right) F_{ij}\mathds{1}(\text{i chooses j}) 
        = \left(S_i- \mathbb{E}^j_{T} \left[\sum_{t=1}^T\frac{F_{ij}}{(1+r_j)^t} \, \middle| \, x_i \right]\right)\mathds{1}(\text{i chooses j}) 
    \end{align*}
\end{itemize}
