...
\begin{itemize}
    \item  In several markets consumers receive initial offers, then they can request revised offers. Example: 
    \begin{itemize}
        \item Loans: consumers get a loan estimate (LE) and showing a LE to another lender could lead to a revised offer 
        \item Website advice: "you can often use competing offers as leverage to negotiate"
    \end{itemize}
...
\begin{itemize}
    ...
    \item Data from a centralized marketplace for annuities in Chile (SCOMP)
    \begin{itemize} 
        \item Contains initial and revised offers
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Incorporate asymmetric information across firms about consumer risk 
    \end{itemize}

    \item Competition in selection markets:
    \textbf{\textcite{cosconati_competing_2025}}, 
    \textcite{mahoney_imperfect_2017, crawford_asymmetric_2018, cuesta_price_2018} 

    \begin{itemize} 
        \item Incorporate a second stage where firms observe intiial offfers of competitors and update their own offers
    \end{itemize}
        
    \item Market design in  selection markets : \textbf{\textcite{aryal_auctioning_2021}}, \textcite{abaluck_when_2023,tebaldi_estimating_2025} 
   
    \begin{itemize}
        \item Study the effect of a particular feature of a market
    \end{itemize}
...
\begin{itemize}
    ...
    \item Buyers draw search cost $s$ from a joint distribution $H(s,\theta)$    \item Timeline:
    \begin{enumerate}
...
\begin{itemize}
    ...
    \begin{align*}
        \lambda(\theta, \hat \theta) &= H_{s\mid \theta}\big(G(\theta, \hat \theta)\big).
    \end{align*}
\end{itemize}
...
\begin{align*}
    ...
    F_j^{2}(\hat \theta) &= \max \{\alpha_j^{2} + \beta_j^{2} \, E[\theta \mid (\hat \theta_j, \hat \theta_{-j}), D=j,t=2], F_j^{1}(\hat \theta_j)\}
\end{align*}
\begin{itemize}
...
\small
\begin{align*}
    \pi_j &= \int_{\theta} \int_{\hat \theta} (W - F_j^{1}(\hat \theta_j)  k_j \theta) \, \Pr(D=j \mid \theta, \hat \theta, t=1) (1-\lambda(\theta, \hat \theta)) \, \phi(\hat \theta; \theta) f_0(\theta) d\hat \theta d\theta \\
    &\quad + \int_{\theta} \int_{\hat \theta} (W - F_j^{2}(\hat \theta) k_j \theta) \, \Pr(D=j \mid \theta, \hat \theta, t=2) \lambda(\theta, \hat \theta) \, \phi(\hat \theta; \theta) f_0(\theta) d\hat \theta d\theta.
\end{align*}
\normalsize
\begin{itemize}
...
\begin{itemize}
    \item A pricing equilibrium consists on $(\alpha_j^t, \beta_j^t)_{t,j}$ such that for all $j$,  $(\alpha_j^t, \beta_j^t)_{t}$ maximizes $\pi_j\left[(\alpha_j^t, \beta_j^t)_{t}; (\alpha_j^t, \beta_j^t)_{t,-j}\right] $ 
\end{itemize}
