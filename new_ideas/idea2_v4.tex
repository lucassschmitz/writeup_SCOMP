...
\noindent We define the index $j \in \{1,2\}$ to refer to the relevant margin. We can summarize (\ref{eq:obs_A}) and (\ref{eq:obs_G}) as:
\begin{equation}
    F_\beta^1(\beta_j(x,r;\theta_1)) = F_\beta^0(\beta_j(x,r;\theta_0)) \quad \text{for } j \in \{1,2\}. \label{eq:obs_equiv}
\end{equation}
...
The left-hand side of (\ref{eq:F_constraint}) depends only on $b$. Consequently, the right-hand side must be invariant to $x$. Since $F_\beta^0$ is strictly increasing, this requires that $\phi_j$ itself must be invariant to $x$:
\begin{equation}
    \frac{\partial \phi_j(b, x; \theta_0, \theta_1)}{\partial x} = 0 \quad \text{for all } x. \label{eq:phi_invariant}
\end{equation}

\noindent \textbf{Formal Argument via Marginal Rates of Substitution:}
...
\end{equation}
This condition states that the Marginal Rate of Substitution (MRS) between survival probability $x$ and price $r$ must be identical for both belief parameters.
Recall that $\hat{x} = \theta x$. The function $\beta_1(x, r; \theta)$ (for example) typically takes the form:
\[ \beta_1 \propto \frac{1 + \theta x + (\theta x)^2}{1 - \theta x} \cdot (F(r)^\alpha - F_g(r)^\alpha) \]
The MRS depends on the derivative of the belief term with respect to $\theta x$. Due to the non-linear way $\theta$ enters the valuation of the annuity (geometric sum) versus the linear bequest, the function defining the MRS is strictly monotonic in $\theta$. Therefore, equation (\ref{eq:MRS_equality}) holds if and only if $\theta_1 = \theta_0$.

If $\theta_1 \neq \theta_0$, then $\partial \phi_j / \partial x \neq 0$, which implies $F_\beta^1(b)$ would vary with $x$—a contradiction to it being a valid distribution of preferences. Thus, $\theta$ is point identified. \qed
...
\begin{align}
    ...
    &= \Pr(D_i = A \mid x_i; \theta,F_\beta)^{\mathds{1}(D_i = A)} \notag \\
    &\quad\times \Pr(D_i = G \mid x_i; \theta,F_\beta)^{\mathds{1}(D_i = G)} \notag \\
    &\quad\times \Pr(D_i = N \mid x_i; \theta,F_\beta)^{\mathds{1}(D_i = N)} \notag \\
    &= p_A(x_i;\theta,F_\beta)^{\mathds{1}(D_i = A)} \notag \\
    &\quad\times p_G(x_i;\theta,F_\beta)^{\mathds{1}(D_i = G)} \notag \\
    &\quad\times p_N(x_i;\theta,F_\beta)^{\mathds{1}(D_i = N)}. \notag
\end{align}
